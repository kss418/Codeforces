...
void run(){
    ...
    for(int i = 1;i <= n;i++){
        ...
        ll lb = *lower_bound(all(es.pn), mn);
        ll mul = lb * i, nxt = mul - sum - ((mul - sum) % i) - (i - 1); 
        if(num.lower_bound(nxt) == num.end()){
            ll pl = *--num.end();
            num.erase(--num.end());
            a[i] = pl; sum += pl; 
        }
        else{
            ll pl = *num.lower_bound(nxt);
            num.erase(num.lower_bound(nxt));
            a[i] = pl; sum += pl; 
        }
    }
